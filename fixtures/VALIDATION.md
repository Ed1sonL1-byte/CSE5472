...
Executed locally on 2026-09-18 America/New_York (2026-09-19 UTC), on Darwin arm64.
...
The concrete tests cover out-of-order no-ops, an incorrect marker followed by
the exact marker, `uint256.max` inputs, accounting/phase invariants, and terminal
state stability. The initial run exposed Foundry's discovery of the application
...
Both Halmos processes exited 0. Raw structured results are retained as
...
These results validate only the fixture behavior and local Halmos invariant
smoke. They do not validate a Medusa corpus adapter, generated prefix harness,
model decoder, cross-engine replay, or any security-vulnerability detection.
