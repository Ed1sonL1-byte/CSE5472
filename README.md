...
Validation records and public evidence are available in:

- [Stage 1 validation](docs/STAGE1_VALIDATION.md)
- [Stage 2 benchmark results](docs/STAGE2_BENCHMARKS.md)
- [Stage 2 validation](docs/STAGE2_VALIDATION.md)
- [Stage 2 evidence index](evidence/stage2/INDEX.md)
- [Stage 3 validation](docs/STAGE3_VALIDATION.md)
- [Stage 3 evidence index](evidence/stage3/INDEX.md)
...
The workflow does not hard-code target solutions. Fixture metadata defines the supported calls, finite-state projection, and goal observer; Medusa supplies the executed prefixes, Halmos supplies candidate values, and a separate concrete replay decides whether a candidate is accepted.
...
- A declared finite-state projection rather than a claim of complete EVM world-state equivalence.

| Fixture | Purpose |
| --- | --- |
| `PhaseCounter` | A narrow integer condition that is difficult for ordinary concrete input generation. |
| `BoundedLedger` | An independent state machine and invariant used to verify that the same core pipeline works across fixtures. |
...
Set an explicit path before bootstrap when solc is installed elsewhere:
...
## Stage 1: run one complete fixture workflow
...
| Option | Meaning |
| --- | --- |
| `--seed` | Seed for the random sources controlled by the adapter. |
| `--warmup-tests` | Maximum number of complete warmup sequences. |
| `--max-prefixes` | Maximum number of concrete prefixes sent to later checks. |
| `--process-timeout` | Outer timeout for one native command. |
| `--total-solve-timeout` | Cumulative solving budget for the augmentation round. |
| `--output` | New output directory. |

Use `./seedbridge run --help` for current defaults. Exhausting a configured budget without a witness means that no witness was found within the bound; it is not a proof of unreachability.

Regenerate the Markdown report for an existing run without rerunning tools:

```sh
./seedbridge report runs/phase-counter-review
```

## Stage 2: campaigns and the 60-record benchmark
...
Raw runs are ignored by Git. The path-sanitized public extract is stored in `evidence/stage2/`.

## Stage 3: plan, run, audit, report, and archive
...
Independently audit compact raw evidence and render the offline reports:
...
The archive contains the frozen configuration and source, compact raw events, accepted native seeds, model and replay evidence, derived reports, per-file checksums, and a standard-library rebuild script. Archive verification safely extracts every member, checks every digest, and rebuilds the report twice without Medusa, Halmos, Foundry, solc, Z3, Go, or network access.
...
The results support a limited conclusion: last-step symbolic augmentation consistently helped the two narrow teaching conditions, while additional native continuation closed most short-budget gaps on the broad and workflow fixtures. The goal-invocation limit comparison did not produce an additional goal hit in this matrix.
...
Seed execution and mutation admission are separate acceptance facts. Admission does not by itself prove that a later mutation was useful. Lineage evidence separately records actual parent selection, a different child, and child execution.

## Evidence and result classification

A Stage 1 run is `stage1_confirmed` only when all of the following hold:

1. Native sequences round-trip losslessly, and observation does not change state, outcomes, coverage feedback, or admission.
2. The selected prefix came from a real Medusa execution and replays with the same actor, context, outcomes, and declared state projection.
...
4. The candidate is not a duplicate of an existing complete sequence and is actually executed by restarted Medusa.
...
| Status | Meaning |
| --- | --- |
| `candidate` | A parseable target model exists but has not completed concrete replay and feedback. |
| `reachable_confirmed` | Fresh concrete replay reached the target; native feedback still needs verification. |
| `result_mismatch` | Process results, test identity, or reported result counts disagree. |
| `tool_error` | A native tool failed; inspect the corresponding log and pinned version. |
| `timeout` / `stuck` / `all_paths_reverted` | Execution or solving did not complete within the configured bound. |
| `no_witness_within_bounds` | No model was found within the configured search bound. |
| `invalid_model` / `decode_error` | The model has the wrong identity, type, name, or range. |
| `state_mismatch` / `observer_mismatch` | Replay state or observation-neutrality evidence differs. |
| `prefix_mismatch` / `replay_mismatch` / `candidate_rejected` | Source, actor, binding, calldata, or step observations did not match. |
| `admission_unverified` | Export or execution occurred, but mutation admission was not established. |

Stage 3 additionally separates valid negative outcomes such as `no_eligible_prefix`, `no_candidate`, bounded augmentation timeout, and native continuation miss from tool, state, and evidence errors. Tool and evidence errors can never be counted as evaluation-valid records.

## Main artifacts

| Artifact | Purpose |
| --- | --- |
| `report.md` / `report.json` | Stage 1 result, configuration, timings, replay evidence, and error classification. |
| Native corpus and seed files | Exact Medusa sequences used for warmup, restart, and import. |
| Normalized prefixes | Provenance, actor, target, calldata, block context, and step observations. |
| Generated Foundry projects | Prefix checks, target solving, and independent concrete replay. |
| Halmos JSON and logs | Target identity, typed model, warnings, bounds, and failure details. |
| Forge and Medusa records | Concrete success, state agreement, target transitions, and native admission. |
| `lineage.jsonl` | Startup replay, new-sequence, mutation, real parent, and execution records. |
| `metrics.json`, `coverage.json`, `states.json` | Separate warmup/import/continuation sequence, state, and runtime-coverage sets. |
| Stage 2 `campaign.json` | Online budget ledger, augmentation status, lineage status, and validity classification. |
| Stage 3 `events.jsonl.gz` | Compact v2 executions, native payloads, prefix references, lineage, timestamps, and EOF integrity. |
| Stage 3 `manifest.json` | Frozen matrix, source and tool hashes, complete blocks, cache protocol, and resume boundary. |
| Stage 3 `derived/audit.json` | Independent audit that does not reuse the production segmented-metrics implementation. |
| Stage 3 `derived/summary.*` and SVG files | Slot observations, paired comparisons, cost breakdown, reasons, and charts. |
| `artifacts/*.tar.gz` | Self-contained offline evidence archives; large archive bodies are not committed. |
...
Audit the saved Stage 3 protocol evidence and verify the public evidence checksums:
...
The latest completed validation recorded 156 passing unit tests, 163 passing tests with integration enabled, a passing Go adapter suite, 320/320 valid formal Stage 3 records, and a successful two-pass offline archive rebuild. See [Stage 3 validation](docs/STAGE3_VALIDATION.md) for the saved command record.
...
| `docs/STAGE1_VALIDATION.md` | Stage 1 acceptance record. |
| `docs/STAGE2_BENCHMARKS.md` | Stage 2 benchmark interpretation and limitations. |
| `docs/STAGE2_VALIDATION.md` | Stage 2 acceptance record. |
| `docs/STAGE3_VALIDATION.md` | Stage 3 acceptance record and final checks. |
